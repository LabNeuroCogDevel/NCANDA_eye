...
dropTrialSacs <- function(subj,runtype,trl,xdatCode,reason,allsacs,showplot=F,savedas=NULL,writetopdf=F,run=0,rundate=0) {
   ...
   for(trial in trl) {
      ...
      cat(sprintf("%s\t%s\n",trial,reason), file=output_filename)
   }
...
#      } else {
#        plot_outputdir <- paste(dirname(savedas),'img',sep='/')
#      }

      if(!file.exists(outputdir)) { dir.create(outputdir,recursive=T) }
      
      filename<-paste(c(subj,"_y",rundate, "_run",run,"_runtype",runtype,'_DROPPED'),sep="", collapse="")
      filename<-paste(c(plot_outputdir,'/', filename,'.pdf'), sep="")
      if(!file.exists(outputdir)) { dir.create(outputdir,recursive=T) }
...
interoplateSamples <- function(trl, funnybusiness=''){
    ...
    if(!any(grepl('dontcutspikes',funnybusiness))){
      e<-length(b.approx$x)
      xpos <- c(0,diff(b.approx$x));
      # change is different on both sides (spike) and is greater than threshold
      spikypointsIdx <- which(rollapply( xpos,2,function(x){sign(prod(x,na.rm=T))==-1 & abs(min(x,na.rm=T))>30}))
...
getSacs <- function(eydfile, subj, run, runtype,rundate=0,onlyontrials=NULL,writetopdf=F,savedas=NULL, showplot=F, funnybusiness=""){
  ...
  for(trl in runontrials ) {
    ...
    if(numtoofarfrombaseline> maxSamplesFromBaseline) {
     # print("numtoofarfrombaseline") # debug
     allsacs <-  dropTrialSacs(subj,runtype,trl,xdatCode,
   ...
   if (   length(b.all$x ) > 200 ) {
    ...
    if(showplot | writetopdf) {
       ...
       filename<-sprintf('%s/%03d_year%d_run%d_trial%02d-%d.pdf',outputdir,subj,rundate,run,trl,xdatCode)
  ...
  if(is.null(onlyontrials) ) {
   ...
   #grep('reason',names(allsacs))
    ...
    number_fields <- which(sapply(allsacs, class)=="numeric")
    allsacs[,number_fields] <- round(allsacs[,number_fields], 3)
   write.table(allsacs, file=savedas, sep="\t",row.names=F, quote=F)
   print('wrote')
  }
